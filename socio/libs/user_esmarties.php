...
class user_esmarties {
	...
	public function load_data($page = null, $rpp = null){
		$query = "SELECT u.username, u.esmarties, u.imagen, u.nombre, u.apellido, u.id_ciudad 
			FROM usuario u 
		...
		if($total > 0){
			...
			if(!empty($ids)){
				$referrals = "SELECT bono_referente as esmarties, 2 as tipo, creado,  NULL as nombre, NULL as link FROM negocio_venta WHERE id_usuario IN ($ids)
					UNION";
			}else{
				$referrals = '';
			}
			$query = "SELECT nv.bono_esmarties as esmarties, 1 as tipo, nv.creado, n.nombre as nombre, n.url as link 
				FROM negocio_venta nv 
				INNER JOIN negocio n ON nv.id_negocio = n.id_negocio
				WHERE nv.id_usuario = :id_usuario1
				UNION 
				$referrals
				SELECT vt.precio as esmarties, 3 as tipo, vt.creado, p.nombre as nombre, vt.id_producto as link 
				FROM venta_tienda vt
				INNER JOIN producto p ON vt.id_producto = p.id_producto
				WHERE vt.id_usuario = :id_usuario2
	...
	public function get_moves(){
		...
		foreach ($this->moves as $key => $value) {
			...
			$link = _safe($value['link']);
			switch ($value['type']) {
				case 1:
					$tag = '<strong class="text-primary mr20">+ '.$esmarties.'</strong> Consumo en <a href="'.HOST.'/'.$link.'" target="_blank">'.$name.'</a>';
					break;
				case 2:
					$tag = '<strong class="text-primary mr20">+ '.$esmarties.'</strong> Bono por referido';
					break;
				case 3:
					$tag = '<strong class="text-danger mr20">- '.$esmarties.'</strong> Compra en tienda: <a href="'.HOST.'/tienda/producto/'.$link.'" target="_blank">'.$name.'</a>';
					break;
				default:
					$tag = '';
					break;
			...
			$html .= 
			'<div class="background-white p20 mb30 text-default clearfix">
				'.$tag.'
				<p class="pull-right">Hace '.$ago.' el '.$date.'</p>
			</div>';
		}
		if(is_null($html)){
			$html = '<div class="background-white p30"><p class="text-default">No tienes movimientos de eSmartties</p></div>';
		}
		return $html;
	}

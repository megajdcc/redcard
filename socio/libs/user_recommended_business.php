...
class user_recommended_business {
	private $con;
	private $user = array(
		'id' => null,
		'recommending' => array()
	...
	public function load_data($page = null, $rpp = null){
		$query = "SELECT COUNT(*) FROM recomendar_negocio WHERE id_usuario = :id_usuario";
		try{
		...
		if($row = $stmt->fetch()){
			...
			while($row = $stmt->fetch()){
				$this->user['recommending'][$row['id_negocio']] = array(
					'name' => $row['nombre'],
					'url' => $row['url'],
	...
	public function get_businesses(){
		$html = null;
		foreach ($this->user['recommending'] as $key => $value) {
		...
		if(is_null($html)){
			$html = '<div class="background-white p20 text-default">No he recomendado a ning&uacute;n negocio.</div>';
		}
		return $html;
	}
	...
	public function get_count(){
		...
		if($i > 0){
			return 'Recomiendo a '.$i.' negocios';
		}else{
			return 'Negocios que recomiendo';
		}
	}
	...
	private function error_log($method, $line, $error){
		file_put_contents(ROOT.'\assets\error_logs\user_recommended_business.txt', '['.date('d/M/Y g:i:s A').' | Method: '.$method.' | Line: '.$line.'] '.$error.PHP_EOL,FILE_APPEND);

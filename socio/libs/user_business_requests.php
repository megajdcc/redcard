...
class user_business_requests {
	...
	public function load_data($page = null, $rpp = null){
		$query = "SELECT COUNT(*) FROM solicitud_negocio WHERE id_usuario = :id_usuario AND mostrar_usuario != 0";
		try{
		...
		if($row = $stmt->fetch()){
			...
			$query = "SELECT s.id_solicitud, s.nombre, s.mostrar_usuario, s.creado 
		...
		foreach ($this->request as $key => $value) {
			...
			$requests .= 
				'<div class="background-white p20 mb30">
					<a href="'.HOST.'/socio/negocios/solicitud/'.$key.'">
		...
		if($requests){
			...
			$html = 
				'<div class="background-white p20 text-default">
					No has enviado ninguna solicitud. <a href="'.HOST.'/socio/negocios/afiliar-negocio">Enviar una</a>.
				</div>';
		}
		return $html;
	}
	...
	public function get_count(){
		...
		if($i > 0){
			...
			return 'Solicitudes enviadas para afiliar un negocio';
		}
	}

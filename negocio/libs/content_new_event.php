...
class content_new_event {
	...
	public function create_event(array $post, array $files){
		...
		if(!array_filter($this->error)){
			if(!move_uploaded_file($this->business['image']['tmp_name'], $this->business['image']['path'])){
				$this->error['notification'] = 'El evento no se ha podido crear correctamente.';
				return false;
			}
			$query = "INSERT INTO negocio_evento (
				id_negocio, 
				titulo, 
				contenido,
				fecha_inicio,
			...
			$_SESSION['notification']['success'] = 'Evento creado exitosamente.';
			header('Location: '.HOST.'/negocio/contenidos/eventos');
			die();
		}
		return;
	}
	...
	private function error_log($method, $line, $error){
		file_put_contents(ROOT.'\assets\error_logs\content_new_event.txt', '['.date('d/M/Y h:i:s A').' | '.$method.' | Line: '.$line.'] '.$error.PHP_EOL,FILE_APPEND);

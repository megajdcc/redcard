...
class personnel_security_code {
	private $con;
	private $business = array('id' => null);
	private $user = array(
		'id' => null,
		'username' => null,
		'email' => null
	...
	public function change_password(array $post){
		...
		if($row = $stmt->fetch()){
			...
			$query = "UPDATE negocio_empleado SET codigo_seguridad = :codigo WHERE id_negocio = :id_negocio AND id_empleado = :id_empleado";
			$params = array(
			...
			$_SESSION['notification']['success'] = 'El código de seguridad ha sido actualizado correctamente.';
			header('Location: '.HOST.'/negocio/personal/codigo-seguridad');
			die();
			return;
		}

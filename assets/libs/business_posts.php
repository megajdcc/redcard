...
class business_posts {
	...
	public function load_data($url, $page = null, $rpp = null){
		...
		$query = "SELECT COUNT(*) FROM negocio_publicacion WHERE id_negocio = :id_negocio";
		try{
		...
		if($row = $stmt->fetch()){
			...
			$query = "SELECT ne.id_publicacion, ne.titulo, ne.contenido, ne.imagen, ne.creado
				FROM negocio_publicacion ne
	...
	public function get_posts(){
		...
		foreach ($this->posts as $key => $value) {
			...
			if($value['image']){
				$image = HOST.'/assets/img/business/post/'._safe($value['image']);
				$image_html =
		...
		if(is_null($html)){
			$html = '<div class="background-white p30"><p class="text-default">Este negocio no ha hecho ninguna publicaci&oacute;n.</p></div>';
		}
		return $html;
	}
	...
	private function error_log($method, $line, $error){
		file_put_contents(ROOT.'\assets\error_logs\business_posts.txt', '['.date('d/M/Y g:i:s A').' | Method: '.$method.' | Line: '.$line.'] '.$error.PHP_EOL,FILE_APPEND);

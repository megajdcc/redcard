...
class recharge_business {
	...
	public function new_balance(array $post){
		...
		if(!array_filter($this->error)){
			...
			$_SESSION['notification']['success'] = 'Se han recargado $'.$this->new_balance['balance'].' de saldo al negocio exitosamente.';
			header('Location: '.HOST.'/admin/negocios/recargar');
			die();
			return;
		}
